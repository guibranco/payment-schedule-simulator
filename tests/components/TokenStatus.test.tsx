...
describe('TokenStatus', () => {
  ...
  it('shows an hours-and-minutes countdown', async () => {
    localStorage.setItem(STORAGE_KEYS.ACCESS_TOKEN, 'token');
    localStorage.setItem(STORAGE_KEYS.TOKEN_EXPIRES_AT, String(Date.now() + 3 * 60 * 60 * 1000));
  ...
  it('shows a days-and-hours countdown', async () => {
    localStorage.setItem(STORAGE_KEYS.ACCESS_TOKEN, 'token');
    localStorage.setItem(STORAGE_KEYS.TOKEN_EXPIRES_AT, String(Date.now() + 30 * 60 * 60 * 1000));
