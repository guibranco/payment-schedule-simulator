 ...
 *
 * This component calculates the total amount of all items in the schedule and displays various details such as collection day,
 * cover period, and schedule ID. It also renders a table with detailed information about each schedule item, including
 * taxes and levies. The component includes buttons to download the full schedule data in JSON or CSV formats.
 *
 * @param schedule - An object containing the schedule's items and metadata.
 */
export default function ScheduleDisplay({ schedule }: Props) {
  ...
  /**
   * Downloads a CSV file containing schedule items details.
   *
   * This function generates a CSV file with headers and rows derived from the `scheduleItems` array.
   * Each row includes formatted date strings, monetary values, and tax/admin fee information.
   * The CSV is created as a Blob, converted to a URL, and then triggered for download via an anchor element.
   */
  const downloadCsv = () => {
